    ...
    <>
      ...
      {!isOpen && (
        <motion.button
          ...
          animate={{ scale: 1 }}
          onClick={onToggle}
          className="fixed bottom-24 right-6 z-50 h-12 w-12 rounded-full bg-zinc-800 border border-zinc-700 shadow-lg shadow-black/30 flex items-center justify-center hover:bg-zinc-700 transition-colors xl:bottom-6"
        >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed right-0 top-16 bottom-20 xl:bottom-0 w-80 z-40 bg-zinc-950/95 backdrop-blur-2xl border-l border-zinc-800/60 flex flex-col shadow-2xl shadow-black/50"
          >
            {/* Header */}
              ...
              <div className="space-y-3">
                {messages.length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-[10px] text-zinc-700 mt-1">Say hello! 👋</p>
                  </div>
                )}
                {messages.map((msg, i) => (
                  <motion.div
                  ...
                  >
                    <Avatar className="h-7 w-7 flex-shrink-0 border border-zinc-800">
            ...
            <div className="p-3 border-t border-zinc-800/60">
              <div className="flex gap-2">
                <Input
                  ...
                  className="bg-zinc-900/60 border-zinc-800/60 text-zinc-200 placeholder:text-zinc-600 h-10 text-sm"
                />
                <Button
                  onClick={handleSend}
                  size="icon"
                  disabled={!input.trim()}
                  className="h-10 w-10 bg-violet-600/80 hover:bg-violet-600 text-white rounded-xl flex-shrink-0 disabled:opacity-30"
                >
